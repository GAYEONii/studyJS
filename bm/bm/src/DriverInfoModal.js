...
export default class DriverInfoModal {
  ...
  constructor($target) {
    ...
    $modal.addEventListener('click', (evt) => {
      if (evt.target.classList.contains('modal')) this.close();
    });
    this.$modal = $modal;
  ...
  render() {
    if (this.data.visible) {
      ...
      const orders = [...driver.orders, ...driver.reservedOrders];
      ...
      this.$orderTable.render(orders, (item) => {
        return `
                    <tr>
                        ...
                        <td>${this.data.places.find(d=>d.id === item.placeId).name}</td>
                        <td>${this.data.places.find(d=>d.id === item.placeId).position.join(', ')}</td>
                        <td>${item.deliveryPosition.join(', ')}</td>
                        <td>${item.price}</td>
                        <td>${item.deliveredAt === '' ? (item.pickedUpAt === ''?` 배달 예정 `:` 배달 중 `) : ` 배달 완료 `}</td>
                    </tr>
                `;
      });
